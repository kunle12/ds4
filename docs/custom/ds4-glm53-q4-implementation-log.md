...
| 6 | `ds4.c:46076` `glm_graph_layer_uses_generic_routed_moe` | the Q4_K branch is unconditional (later guarded on streaming in `44fc48f`) | the Q4_K-specific kernels were the default and measured **2.7× slower** than the pre-existing generic dispatch at identical output, so the generic one was promoted; the ported kernels are exercised by `make test-glm53-moe-q4k`, because `DS4_CUDA_GLM_MOE_TYPES` is read inside the GLM-specific dispatch and cannot re-route a homogeneous Q4_K trio to it, and (since `44fc48f`) are also reached at runtime by a streaming graph |
